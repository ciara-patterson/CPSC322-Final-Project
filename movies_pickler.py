...
# get the train and test set for linear_regression
# X_train = [features[j] for j in range(len(features)) if j in X_train_folds_strat[0]]
# y_train = [outcomes[j] for j in range(len(outcomes)) if j in X_train_folds_strat[0]]
# X_test = [features[j] for j in range(len(features)) if j in X_test_folds_strat[0]]
# y_test = [outcomes[j] for j in range(len(outcomes)) if j in X_test_folds_strat[0]]

# fit the linear regression model
kn_class.fit(features, outcomes)
# test_folds(k, X_train_folds_strat, X_test_folds_strat, kn_class)
# print(kn_class.X_train)
